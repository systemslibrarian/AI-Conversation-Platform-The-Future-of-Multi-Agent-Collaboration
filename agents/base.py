...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        log_event(self.logger, "agent_initialized", {
    ...
    async def generate_response(self) -> Tuple[str, int, float]:
        ...
        if self.circuit_breaker.is_open():
            raise Exception(f"Circuit breaker OPEN. Retry in {int(self.circuit_breaker.timeout_seconds - (time.time() - (self.circuit_breaker.last_failure_time or 0)))}s")
        ...
        with self.tracer.start_as_current_span(f"{self.PROVIDER_NAME}.generate"):
            try:
                ...
                log_event(self.logger, "response_generated", {
                    "agent": self.agent_name,
                    "tokens": tokens,
                ...
                error_type = "rate_limit" if ("rate" in str(e).lower() or "429" in str(e)) else "api_error"
                record_error(self.PROVIDER_NAME, error_type)
                record_call(self.PROVIDER_NAME, self.model, "error")
                
                log_event(self.logger, "api_error", {
                    "agent": self.agent_name,
                    "error": str(e),
    ...
    async def run(self, max_turns: int, partner_name: str) -> None:
        ...
        log_event(self.logger, "agent_started", {
            "agent": self.agent_name,
            "max_turns": max_turns
        })
        ...
        try:
            ...
            log_event(self.logger, "agent_error", {
                "agent": self.agent_name,
                "error": str(e)
            })
            print(f"\n✗ Fatal error: {e}")
