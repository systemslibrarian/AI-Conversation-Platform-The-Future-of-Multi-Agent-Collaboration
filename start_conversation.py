...
class ConversationStarter:
    ...
    def _check_configuration(self) -> bool:
        ...
        if len(self.available_agents) < 2 and not (self.args and self.args.agent1 and self.args.agent2):
    ...
    def _print_summary(self, agent1_type, agent1_model, agent2_type, agent2_model, topic, max_turns):
    ...
    async def run_interactive(self):
        ...
        agent1_type, agent1_model = self._select_agent(
            "first", 
            self.args.agent1 if self.args else None
        )
        agent2_type, agent2_model = self._select_agent(
            "second", 
            self.args.agent2 if self.args else None
        )
        ...
        await self.run_conversation(agent1_type, agent1_model, agent2_type, agent2_model, topic, max_turns)
...
Examples:
  ...
  aic-start --db ./conversations/my_chat.db
    ...
    parser.add_argument("--agent1", type=str, help="First agent (claude, chatgpt, gemini, grok, perplexity)")
