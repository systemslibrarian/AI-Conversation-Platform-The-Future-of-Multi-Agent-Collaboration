...
class DummyOpenAIClient:
    def __init__(self, *args, **kwargs):
        ...
        def _create(**kwargs):
            return type("Resp", (), {"choices": [DummyChoice("Hello, world!")], "usage": DummyUsage(20)})()
...
async def test_chatgpt_api_call(mock_queue, logger, monkeypatch):
    ...
    with patch("agents.chatgpt.OpenAI", DummyOpenAIClient):
        agent = ChatGPTAgent(api_key="dummy", queue=mock_queue, logger=logger, model="gpt-4o", topic="x", timeout_minutes=1)
...
async def test_claude_api_call(mock_queue, logger, monkeypatch):
    ...
    with patch("agents.claude.anthropic.Anthropic", DummyAnthropicClient):
        agent = ClaudeAgent(api_key="dummy", queue=mock_queue, logger=logger, model="claude-sonnet-4-5-20250929", topic="x", timeout_minutes=1)
        content, tokens = await agent._call_api([{"role": "user", "content": "Hi"}])
