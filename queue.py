...
class SQLiteQueue:
    ...
    def __init__(self, filepath: Path, logger, lock_timeout: int = 30):
        ...
        log_event(self.logger, "queue_initialized", {"filepath": str(self.filepath), "type": "sqlite"})
            ...
            with self.lock:
                ...
                try:
                    ...
                    cursor = conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (msg["sender"], msg["content"], msg["timestamp"], msg["hash"], msg["metadata"]))
                    ...
                    log_event(self.logger, "message_added", {
                        "id": msg_id,
                        "sender": sender,
                        "length": len(content)
                    })
        ...
        try:
            with self.lock:
                ...
                try:
                    ...
                    conn.execute("UPDATE metadata SET value = ? WHERE key='termination_reason'", (reason,))
                    conn.execute("INSERT OR REPLACE INTO metadata (key, value) VALUES ('ended_at', ?)", (now,))
                    conn.commit()
    ...
    async def get_termination_reason(self) -> str:
        ...
        conn = sqlite3.connect(str(self.filepath))
        try:
            row = conn.execute("SELECT value FROM metadata WHERE key='termination_reason'").fetchone()
